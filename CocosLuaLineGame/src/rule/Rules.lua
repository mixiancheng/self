...
--_colorTable={normal=cc.c4f(0,0.5,0.5,1),dead=cc.c4f(0.5,0.5,0,1),factory=}
...
function isMoveOver() --@return typeOrObject
    for k,v in ipairs(_mapData) do
        if v.obj~=nil then
            --            cclog("_col==%d,_row==%d,_type==%s,state==%d",v._col,v._row,v._type,v.obj._state)
            if v.obj._state==ObjState.droping then
                --            cclog("falese------------->")
                --        cclog("_col==%d,_row==%d,_type==%d",v._col,v._row,v._type)
                return false end
        end
end
return true
...
function OnTouchEnd(touch) --@return typeOrObject
    ...
    cclog("-->")
    bombObjs(_pushObjs)
function sortMapData(a,b) --@return typeOrObject
    ...
    table.sort(_mapData,sortMapData)
    for k,v in ipairs(_mapData) do
        if v.obj~=nil  then
            v.obj._state= ObjState.normal
        end
    end
end